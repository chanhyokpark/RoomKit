...
export const connectionTools = [
  ...
  defineTool({
    ...
    inputSchema: z.object({
      themeId: z.string().min(1).optional().describe('Theme uuid (or name — same as `name`)'),
      name: z.string().min(1).optional().describe('Theme name (exact, or unique case-insensitive match)'),
    }),
    handler: async ({ themeId, name }, ctx) => {
      const ref = themeId ?? name;
      if (!ref) throw new ToolError('Pass themeId or name.');
      const themes = await ctx.api.api('/themes', { schema: z.array(ThemeSchema) });
      const theme = matchTheme(themes, ref);
