<!--
  ...
  the code through the hintphone connection; the reply lands in the context
...
<script lang="ts">
	...
	const disabled = $derived(
		ctx.snapshot.pending ||
			(ctx.snapshot.connectionState !== 'connected' &&
				ctx.snapshot.connectionState !== 'needs-code')
	);
	...
	function submit(code: string) {
		...
		else ctx.controller?.submitCode(trimmed);
		value = '';
	}
