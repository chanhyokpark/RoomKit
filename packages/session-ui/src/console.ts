import { CommandSchema, type Asset, type AssetKind, type Command } from '@roomkit/shared';
...
export function parseConsole(input: string, assets: Asset[]): ConsoleResult {
	...
	if (commandName === 'list') {
		...
		if (!kind) throw new ConsoleError('사용법: list devices|players|events|phases|hints|…');
		const rows = assets.filter((asset) => asset.kind === kind);
		return {
			output: rows.length > 0 ? rows.map((asset) => `${asset.name} · ${asset.id}`) : ['(없음)']
		};
	}
	if (commandName === 'json') {
		try {
			...
			throw new ConsoleError(
				error instanceof Error ? error.message : 'Command JSON이 올바르지 않습니다.'
			);
		}
	}
	...
	switch (commandName) {
		...
		case 'navigate':
			return command({
				...
				query: args.slice(2).map((pair) => {
					...
					if (index < 1) throw new ConsoleError(`key=value 형식이 아닙니다: ${pair}`);
		...
		case 'adjusttimer': {
			...
			if (!value) throw new ConsoleError('사용법: adjustTimer +30s|-1m|pause|resume');
...
function find(assets: Asset[], kind: AssetKind, token: string | undefined): Asset {
	if (!token) throw new ConsoleError(`${kind} 이름을 입력하세요.`);
	const pool = assets.filter((asset) => asset.kind === kind);
	const exact = pool.find(
		(asset) => asset.id === token || asset.name.toLowerCase() === token.toLowerCase()
	);
	if (exact) return exact;
	const partial = pool.filter((asset) => asset.name.toLowerCase().includes(token.toLowerCase()));
	if (partial.length === 1) return partial[0];
	if (partial.length > 1)
		throw new ConsoleError(`여러 애셋이 일치합니다: ${partial.map((a) => a.name).join(', ')}`);
...
function duration(token: string): number {
	...
	return Math.round(match[2] === 'ms' ? value : match[2] === 'm' ? value * 60_000 : value * 1000);
}
