...
`rk init` 으로 만든 프로젝트라면 `rk deploy` 한 번으로 빌드·압축·업로드하고 website 애셋을 **ZIP 호스팅** 모드로 전환합니다. 직접 배포하려면 `dist/` 내용물을 ZIP으로 압축해 `index.html`이 루트에 오도록 Studio의 **ZIP 호스팅** 웹사이트로 올리거나, 정적 호스팅 주소를 **외부 URL** 웹사이트로 등록해 주세요.
