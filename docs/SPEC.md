...
| Send message to device | device, message, values, waitUntilEnd | builds the payload from the message asset's field schema + values entered in the editor, then sends it to the device. Tauri relays it to the iframe via postMessage. String values support `{{vars.x}}` / `{{payload.x}}` interpolation; a value that is exactly one template substitutes the variable's raw JSON value (type-preserving), mixed strings stringify, unresolved exact templates become null. With `waitUntilEnd` the wire carries `awaitHandled` and the device defers its ack until every `message` listener's returned promise settles (helper pages answer via a `message:done` postMessage), so the sequence waits on the site's handler |
...
The API is a subset of the client library: `trigger()`, `triggerAndWait()` (resolves when the trigger's event runs finish; the player relays the server's trigger ack as a `trigger:result`), `on('message')` (listeners may return a promise; for a send-message command with waitUntilEnd the player forwards a `commandId` and the helper answers `message:done` once all listeners settle — that gates the command's ack), plus the hint surface (`submitHint()`, `requestHintStep()`, `on('hint')`, `on('hintError')`). The postMessage envelopes live in `@roomkit/shared` (`helper.ts`); the iife bundle (`dist/roomkit-helper.global.js`, ~1.5KB, defines `window.RoomKitHelper`) does structural checks only, while the player validates with the zod schemas.
