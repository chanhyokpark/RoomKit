...
- `bgmVolume(cmd)`: 플레이어별 BGM 기본 볼륨(`cmd.value`, 0~1)을 적용하고 reset까지 유지해 주세요. `cmd.durationMs`가 0보다 크면 현재 볼륨에서 목표까지 그 시간 동안 선형으로 변화시키고, 0이면 즉시 적용합니다. 페이드와 덕킹은 이 값에 곱해서 적용합니다.
