...
힌트 입력, 단계 이동, 정답과 운영자 푸시는 실행 중인 테스트 세션에서 검증해 주세요. Player 런처의 **테스트** 탭으로 세션을 만들면 디버그 창에서 힌트 푸시와 힌트 코드 표시·숨김도 바로 시험할 수 있습니다. 운영자 푸시는 현재 Studio 카드가 아니라 디버그 창, CLI 의 `rk session hint <세션> <힌트>` 또는 `POST /api/sessions/:id/hint`로 사용할 수 있습니다.
