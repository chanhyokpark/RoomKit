...
| `apps/cli`             | The `rk` CLI: project init/deploy plus every Studio operation for humans and AI agents through REST and virtual device sockets (`apps/mcp` is deprecated).      |
...
- Authentication is a single administrator account with bearer JWT for Studio/CLI operations.
...
- The CLI stores admin credentials in `~/.roomkit/mcp-credentials.json` (plaintext, mode 600) for automatic login. Anyone who can run `rk` on that machine effectively has Studio administrator access.
